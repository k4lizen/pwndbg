...
arch_to_reg_const_map = {
    ...
    "x86-64": create_reg_to_const_map(
        ...
        {
            "FSBASE": U.x86_const.UC_X86_REG_FS_BASE,
            "GSBASE": U.x86_const.UC_X86_REG_GS_BASE,
        },
    ),
...
class Emulator:
        ...
        try:
            ...
            if e.errno == U.UC_ERR_READ_UNMAPPED:
                try:
                    ...
                    debug(
                        DEBUG_MEM_READ,
                        "Emulator failed to read memory at %#x, %r",
                        (address, e),
                    )
    ...
    def map_page(self, page) -> bool:
        ...
        try:
            ...
            debug(
                DEBUG_MEM_MAP,
                "Could not map page %#x during emulation! [exception]",
                page,
            )
            return False
        ...
        if not data:
            debug(
                DEBUG_MEM_MAP,
                "Could not map page %#x during emulation! [no data]",
                page,
            )
            return False
    ...
    def until_jump_hook_code(self, _uc, address, instruction_size: int, _user_data) -> None:
        ...
        # The previous instruction does not immediately precede this one.
            ...
            debug(
                DEBUG_EXECUTING,
                "%#x %#X --> %#x",
                (self._prev, self._prev_size, self._curr),
            )
            self.emu_stop()
            return
    ...
    def until_syscall_hook_code(self, uc, address, size: int, user_data) -> None:
        ...
        debug(
            DEBUG_EXECUTING,
            "# Executing instruction at %(address)#x with bytes %(data)s",
            locals(),
        )
