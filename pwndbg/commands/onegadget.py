...
parser.add_argument(
    "--show-unsat", help="Show unsatisfiable gadgets.", action="store_true"
)
parser.add_argument(
    "--no-unknown", help="Do not show unknown gadgets.", action="store_true"
)
    ...
    if not gadgets_count[pwndbg.aglib.onegadget.SAT] and not show_unsat:
        print(
            M.warn(
                ...
                " to check unsatisfiable gadgets.\nTo see why they are unsatisfiable,"
                " you might want to run with -v or --verbose."
            )
        )
