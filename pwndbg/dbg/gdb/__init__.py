...
class GDB(pwndbg.dbg_mod.Debugger):
    ...
    # should also reset when clear screen config is modified
    ...
    def set_prompt(self) -> None:
        ...
        # FIXME: do we need to readline_escape this? would make sense..
        # print("leftover: ", leftover_lines)
        # one line for the unaccounted for pwndbg>
        # one line for the "0x000055555556fa8f in main ()"-type message
        # one line for the newline generated by pressing enter
        # = 3
        print("lns: ", pwndbg.commands.context.ctx_leftover_lines)
        if pwndbg.commands.context.ctx_leftover_lines >= 4 and not pwndbg.commands.context.config_clear_screen:
            prompt += "\x01\x1b[2S\x1b[2A\x02"

        # FIXME: ^this gets messed up when gdb decides to print a lot of stuff (>1 lines), like e.g. on Ctrl-C
        # Program received signal SIGINT, Interrupt.
        # 0x00007ffff7e63cde in pselect64_syscall (nfds=1, readfds=0x7fffffffcce0, writefds=0x0, exceptfds=0x0, timeout=<optimized out>, sigmask=0x7ffff7f97de0 <_rl_orig_sigset>) at ../sysdeps/unix/sysv/linux/pselect.c:34
